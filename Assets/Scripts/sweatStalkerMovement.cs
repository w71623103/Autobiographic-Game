...
using UnityEngine;
using System;
public class sweatStalkerMovement : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        {
            collision.gameObject.GetComponent<playerMood>().mood -= 3;
        }
    }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject.tag == "Player")
        {
            collision.gameObject.GetComponent<playerMoving>().mood -= 1;
        }
    }*/
}
